...
function find_v0_recovery(number_slices, number_nodes, nodes_state, total_cpus_clocks, adjacency_matrix, total_throughput, number_VNFs, number_cycles, traffic, β, node_recovery_requirements, θ)
    ...
    @objective(model, Min, sum(recovery_objective_function(s, number_slices, number_nodes, number_VNFs, number_cycles, traffic, clocks, throughput, VNFs_placements, Virtual_links, β, zeros(number_nodes), node_recovery_requirements, θ) for s in 1: number_slices)) 
