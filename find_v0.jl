...
function find_v0(number_slices, number_nodes, total_cpus_clocks, adjacency_matrix, total_throughput, number_VNFs, number_cycles, traffic, delay_tolerance)
    ...
    initial_cycles = [0.01 for _ in 1: number_slices, _ in 1: number_VNFs]
    initial_throughput = [1 for _ in 1: number_slices, _ in 1: number_VNFs - 1]

    alpha = 0.5

    @objective(model, Min, sum(alpha * (sum(initial_cycles[s, k] * VNFs_placements[s, k, c] for k in 1: number_VNFs for c in 1: number_nodes) + 
    sum(initial_throughput[s, k] * Virtual_links[s, k, i, j] for k in 1: number_VNFs - 1 for i in 1: number_nodes for j in 1: number_nodes)) + 
    (1 - alpha) * (sum(number_cycles[s, k] / initial_cycles[s, k] * VNFs_placements[s, k, c] for k in 1: number_VNFs for c in 1: number_nodes) + 
    sum(traffic[s, k] / initial_throughput[s, k] * Virtual_links[s, k, i, j] for k in 1: number_VNFs - 1 for i in 1: number_nodes for j in 1: number_nodes))
    for s in 1: number_slices)) 
